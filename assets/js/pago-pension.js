document
  ...
  .addEventListener("submit", async function (e) {
    ...
    doc.setFontSize(12);
    doc.text("Comprobante de Pago de Pensión", 105, 30, null, null, "center");
    doc.text(`Fecha: ${fecha}`, 20, 45);
    doc.text(`Alumno: ${nombre}`, 20, 55);
    doc.text(`Grado: ${grado}`, 20, 65);
    doc.text(`Mes pagado: ${mes}`, 20, 75);
    doc.text(`Monto de Pensión: S/ ${monto}`, 20, 85);
    doc.text(`Método de Pago: ${pago}`, 20, 95);

    doc.setFont("times", "italic");
    doc.setTextColor(60);
    doc.text("Gracias por su puntualidad en el pago.", 20, 115);
    ...
    doc.addImage(qrImage, "PNG", 150, 50, 40, 40);

    // Generar número de pago único (ejemplo: PAY-20250710-8431)
    ...
    const numeroPago = `PAY-${fechaCodigo}-${aleatorio}`;

    doc.setFontSize(10);
    doc.setTextColor(50);
    doc.text(`N° de Pago: ${numeroPago}`, 190, 30, null, null, "right");
    ...
    doc.setFontSize(9);
    doc.setTextColor(80);
    doc.text(`TÉRMINOS Y CONDICIONES`, 105, 130, null, null, "center");
    const condiciones = [
      "1. Este comprobante es una constancia de pago válido para el mes indicado.",
    ...
    doc.save(`Comprobante_${nombre}_${mes}.pdf`);
  });
